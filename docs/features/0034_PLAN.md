...
| UTA cross-margin liq diverges because replay seeds per-coin `wallet_balance` instead of account-level available balance | Feature 0042 adds wallet account fields and feeds `total_available_balance` into `_estimate_liquidation_price`. Buy-side hedge-credit behavior remains scoped to 0043. |
