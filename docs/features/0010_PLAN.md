...
## Config File: `conf/pnl_checker.yaml`
...
**Funding calculation**:
- From API: sum of `funding` field from transaction log (already actual)
- Our calculation: `size * mark_price * funding_rate` (current snapshot, for reference)
...
| Cumulative Funding | sum of transaction log | our `size * mark * rate` snapshot |
...
```
python -m pnl_checker.main --config conf/pnl_checker.yaml [--tolerance 0.01] [--output output/] [--debug]
```
