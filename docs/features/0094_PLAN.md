...
- **O1 — RESOLVED by feature 0102 / issue #246: account-wide halt over the
  shared MM pool.**
  `MultiReplayEngine` now observes `pool = total_equity - total_mm` before
  fills and after the account sample, latches the first non-positive active
  pool, and freezes non-reduce-only placements across every runner. It reuses
  the existing 0045 hedge-aware `total_mm` fed by cached `last_price` (the
  established mark proxy), rather than adding a distinct mark feed. The
  resulting counterfactual cut-off is exposed as `liquidated`, `liq_ts`, and
  `min_account_pool` on `MultiReplayResult`.

  The original limitation was:
  ...
  free pool and under-stating joint liq risk. Feature 0102 resolves the
  account-survival gap with a shared-MM halt; the per-runner liquidation-price
  estimate remains symbol-local, so it is not a substitute for full
  account-level liquidation-price fidelity.
