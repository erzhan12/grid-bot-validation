...
> **Note (implementation divergence):** `initial_balance`, `enable_funding`, and `wind_down_mode` live at the **root level** of `ReplayConfig`, not nested under `strategy`. This is intentional — they are backtest/replay parameters, not grid-strategy parameters. The YAML example in `conf/replay.yaml.example` reflects the actual root-level layout.
...
# Backtest/replay parameters (root-level, not nested under strategy)
