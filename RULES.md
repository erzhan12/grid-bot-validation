...
10. **Active WS reconnect with TCP-level probe (2026-05-03, feature 0024)**: bbu2 `_ensure_*_connection` pattern
   ...
   - **Heartbeat thread sharp edge**: `on_disconnect` callback runs on the heartbeat thread. Calling `reset()` directly would cause `Thread.join()` on current thread → `RuntimeError`, plus zombie heartbeat. **Fix**: `_on_ws_disconnect` dispatches `reset()` to a one-shot daemon worker thread (`WSReset-{account}-{kind}`).
