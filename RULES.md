...
7. **orderLinkId Wire Format & Matching (HOTFIX 2026-05-08)**: How the deterministic `client_order_id` survives Bybit's id-cache
   ...
   - **Retry idempotency invariant (2026-05-09 hotfix)**: A fresh suffix is assigned once per concrete placement lifecycle, then stored on `PlaceLimitIntent.order_link_id`. Reattempts of the same failed placement (runner immediate reattempts or `RetryQueue` retries) MUST reuse that same wire `orderLinkId`; generating a new suffix for an ambiguous failure (e.g., HTTP timeout after Bybit accepted the order) defeats exchange idempotency and can create duplicate live orders. New logical placements after cancel/fill still get a fresh suffix.
   ...
   - **Files**: `packages/gridcore/src/gridcore/intents.py` (helper + `PlaceLimitIntent.order_link_id`), `apps/gridbot/src/gridbot/executor.py` (wire-id creation/fallback), `apps/gridbot/src/gridbot/runner.py` (assign/reuse wire id + read-side normalization), `apps/comparator/src/comparator/loader.py`, `apps/replay/src/replay/snapshot_loader.py`.
