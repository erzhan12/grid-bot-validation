...
5. **Reconciliation**
   ...
   - **Risk**: If manual orders are placed on the same symbol while the strategy is running, they will be incorrectly tracked by the strategy, potentially causing incorrect grid state. To safely manually intervene, stop the strategy first, make manual changes, then restart.
   - **CRITICAL**: Before starting the bot for the first time on an account that has been used for manual trading, either (1) close all existing orders for the symbol, or (2) set `allow_shared_symbol: true` and understand that order ownership cannot be guaranteed.
