...
class TestGridBasic:
    ...
    def test_build_greed_basic(self):
        ...
        assert len(grid.greed) == 51

        # Count sides
        buy_count = sum(1 for g in grid.greed if g['side'] == grid.BUY)
        sell_count = sum(1 for g in grid.greed if g['side'] == grid.SELL)
        wait_count = sum(1 for g in grid.greed if g['side'] == grid.WAIT)

        assert buy_count == 25
        assert wait_count == 1
        assert sell_count == 25

    def test_build_greed_price_rounding(self):
    ...
    def test_build_greed_none_price(self):
        """Don't build grid if last_close is None."""
        grid = Grid(tick_size=Decimal('0.1'), greed_count=50, greed_step=0.2)
        grid.build_greed(None)

        assert len(grid.greed) == 0
...
class TestGridUpdate:
    ...
    def test_update_greed_side_assignment(self):
        ...
        filled_level = next((g for g in grid.greed if abs(g['price'] - 99800.0) < 1), None)
        assert filled_level is not None
        assert filled_level['side'] == grid.WAIT

        # Verify prices still sorted (basic validity check)
        # Note: After fills, multiple WAIT levels are expected, so is_greed_correct() may be False
        assert grid.is_price_sorted() is True

    def test_update_greed_rebuilds_on_out_of_bounds(self):
        """Grid rebuilds if price moves outside bounds."""
        grid = Grid(tick_size=Decimal('0.1'), greed_count=50, greed_step=0.2)
        grid.build_greed(100000.0)

        # Store original bounds
        original_min = min(g['price'] for g in grid.greed)
        original_max = max(g['price'] for g in grid.greed)

        # Move price way outside bounds (20% move on 0.2% step grid)
        new_price = 120000.0
        grid.update_greed(last_filled_price=110000.0, last_close=new_price)

        # Grid should rebuild around new price
        new_min = min(g['price'] for g in grid.greed)
        new_max = max(g['price'] for g in grid.greed)
        ...
        middle_prices = [g['price'] for g in grid.greed if g['side'] == grid.WAIT]
    ...
    def test_update_greed_none_handling(self):
        """Don't update grid if prices are None."""
        grid = Grid(tick_size=Decimal('0.1'), greed_count=50, greed_step=0.2)
        grid.build_greed(100000.0)
        original_len = len(grid.greed)

        # None values should be no-op
        grid.update_greed(None, 100000.0)
        assert len(grid.greed) == original_len

        grid.update_greed(99000.0, None)
        assert len(grid.greed) == original_len
...
class TestGridRebalancing:
    ...
    def test_center_grid_buy_heavy(self):
        ...
        buy_before = sum(1 for g in grid.greed if g['side'] == grid.BUY)
        sell_before = sum(1 for g in grid.greed if g['side'] == grid.SELL)

        # Trigger rebalance via update
        grid.update_greed(last_filled_price=99000.0, last_close=100000.0)
    ...
    def test_center_grid_sell_heavy(self):
        ...
        # Trigger rebalance via update
        grid.update_greed(last_filled_price=101000.0, last_close=100000.0)
...
class TestGridHelpers:
    ...
    def test_is_too_close(self):
        """Price within greed_step/4 should return True."""
        grid = Grid(tick_size=Decimal('0.1'), greed_count=50, greed_step=0.2)
    ...
    def test_round_price(self):
        ...
        grid_fine = Grid(tick_size=Decimal('0.01'), greed_count=50, greed_step=0.2)
...
class TestGridProperties:
    ...
    def test_min_max_greed(self):
        ...
        min_price = grid._Grid__min_greed
        max_price = grid._Grid__max_greed

        # Min should be first price
        assert min_price == grid.greed[0]['price']
        # Max should be last price
        assert max_price == grid.greed[-1]['price']
    ...
    def test_buy_sell_counts(self):
        ...
        buy_count = grid._Grid__greed_count_buy
        sell_count = grid._Grid__greed_count_sell
...
class TestGridRebuild:
    ...
    def test_build_greed_clears_before_building(self):
        """Calling build_greed multiple times doesn't double the grid."""
        grid = Grid(tick_size=Decimal('0.1'), greed_count=50, greed_step=0.2)
    ...
    def test_rebuild_centers_on_new_price(self):
        ...
        grid.build_greed(100000.0)
        wait_levels = [g for g in grid.greed if g['side'] == grid.WAIT]
        assert abs(wait_levels[0]['price'] - 100000.0) < 1

        # Rebuild at new price
        grid.build_greed(120000.0)
        wait_levels = [g for g in grid.greed if g['side'] == grid.WAIT]
...
class TestGridEdgeCases:
    ...
    def test_small_tick_size(self):
        ...
        grid = Grid(tick_size=Decimal('0.00001'), greed_count=50, greed_step=0.2)
        grid.build_greed(1.5)

        assert len(grid.greed) == 51
        assert grid.is_price_sorted() is True

    def test_large_greed_count(self):
        """Grid works with large greed counts."""
        grid = Grid(tick_size=Decimal('0.1'), greed_count=200, greed_step=0.2)
        grid.build_greed(100000.0)

        assert len(grid.greed) == 201  # 100 buy + 1 wait + 100 sell
        assert grid.is_price_sorted() is True

    def test_small_greed_step(self):
        """Grid works with very small step sizes."""
        grid = Grid(tick_size=Decimal('0.1'), greed_count=50, greed_step=0.01)
        grid.build_greed(100000.0)
        ...
        prices = [g['price'] for g in grid.greed]
