...
class TestRetryQueuePaused:
    ...
    def test_process_due_drops_duplicate_order_failure(self, place_intent):
        """Duplicate-order-blocked retries are removed, not re-backed-off."""
        executor = Mock(
            return_value=OrderResult(
                success=False, error="duplicate_order_blocked"
            )
        )
        queue = RetryQueue(
            executor_func=executor,
