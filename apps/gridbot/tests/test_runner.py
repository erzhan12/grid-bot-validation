...
class TestSameOrderDetection:
    ...
    def test_non_duplicate_cancel_does_not_drain_queued_retry(
        self, strategy_config, mock_executor, instrument_info
    ):
        """Only reason='duplicate' cancels drain; other reasons leave the queue."""
        retry_queue = RetryQueue(executor_func=mock_executor.execute_place)
        runner = StrategyRunner(
            strategy_config=strategy_config,
            executor=mock_executor,
    ...
    def test_failed_duplicate_cancel_does_not_drain_queued_retry(
        self, strategy_config, mock_executor, instrument_info
    ):
        """A failed duplicate cancel must not drain the queue (order still live)."""
        mock_executor.execute_cancel.return_value = OrderResult(
            success=False, order_link_id="", error="cancel failed"
        )
        retry_queue = RetryQueue(executor_func=mock_executor.execute_place)
        runner = StrategyRunner(
            strategy_config=strategy_config,
...
class TestSafetyCapsIntegration:
    ...
    def test_c3_trip_clears_retry_queue(
        self, strategy_config, mock_executor, instrument_info
    ):
        caps = SafetyCaps(
            SafetyCapsConfig(session_loss_limit="25"), strat_id="btcusdt_test"
        ...
        retry_queue = RetryQueue(executor_func=mock_executor.execute_place)
        r = self._runner(
            strategy_config, mock_executor, instrument_info, caps,
            on_retry_queue_clear=retry_queue.clear,
        )
        retry_queue.add(self._open(), "network error")
        assert retry_queue.size == 1
    ...
    def test_retry_dispatch_place_blocks_when_loss_breaker_latched(
        ...
        caps.check_loss_breaker(
            session_realized_pnl=Decimal("-30"), now_utc=datetime.now(UTC)
        )
        intent = self._assign_wire(self._open())
        result = r.retry_dispatch_place(intent)
        assert result.success is False
        assert result.error == "safety_cap_loss_breaker"
        mock_executor.execute_place.assert_not_called()

    def test_retry_dispatch_place_blocks_c1_notional(
        self, strategy_config, mock_executor, instrument_info
    ):
        caps = SafetyCaps(
            SafetyCapsConfig(max_notional_per_symbol="500"), strat_id="btcusdt_test"
    ...
    def test_retry_dispatch_place_submits_when_caps_allow(
        ...
        caps = SafetyCaps(SafetyCapsConfig(), strat_id="btcusdt_test")
        r = self._runner(strategy_config, mock_executor, instrument_info, caps)
        intent = self._assign_wire(self._open())
        r._tracked_orders[intent.client_order_id] = TrackedOrder(
            client_order_id=intent.client_order_id, intent=intent, status="failed"
        )
        mock_executor.execute_place.return_value = OrderResult(
            success=True, order_id="live_1", order_link_id=intent.order_link_id
        )
        result = r.retry_dispatch_place(intent)
    ...
    def _assign_wire(intent: PlaceLimitIntent) -> PlaceLimitIntent:
        return replace(intent, order_link_id=f"{intent.client_order_id}-123")
