...
class TestReplayEngineSeedingPipeline:
    ...
    def test_load_seed_uses_active_gridbot_run_when_account_scope_misses(
        self, seeded_db, replay_config, seed_ts, snapshot_ts, mock_instrument,
    ):
        """Legacy/shared Phase 4: grid snapshots may use a different account_id.

        When ``seed.account_id`` matches recorder rows but not the gridbot
        writer's ``account_id``, the account-scoped lookup misses and must not
        fall through to ``grid_state_path`` while a live/shadow row exists.
        """
        live_grid = [
            {"side": "Buy", "price": 91000.0},
            {"side": "Buy", "price": 91200.0},
        ...
        assert grid_seed.grid[0]["price"] == 91000.0
